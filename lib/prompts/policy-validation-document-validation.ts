...
3.  **Comprehensive Document Discovery**: Perform a thorough search to find any additional relevant policy documents not on our current list. Where relevant, include platform-specific US-based or global policies that are strongly related (for example, the TAKE IT DOWN Act) - but only when a valid URL is returned by your web searches. Do NOT hallucinate web pages/URLs that do not exist - only use URLs returned from web searches.
