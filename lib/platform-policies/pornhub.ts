...
const termsOfServiceDocument: PolicyDocument = {
  ...
  policies: pornhubPolicies.filter(p => p.reference?.startsWith('PH-TOS')),
  appealProcess: {
...
const trustAndSafetyDocument: PolicyDocument = {
  ...
  accessTimestamp: '2025-06-06T00:00:00Z',
  policies: pornhubPolicies.filter(p => p.reference?.startsWith('PH-POLICY')),
  appealProcess: null
};
