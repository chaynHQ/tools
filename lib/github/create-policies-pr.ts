...
import { ValidationSession, PolicyUpdate } from '@/lib/ai/policy-validation';
...
export function applyPolicyUpdates(platformPolicy: PlatformPolicy, updatedPolicies: PolicyUpdate[]): boolean {
  ...
  updatedPolicies.forEach((updatedPolicy: any) => {
    ...
    const generalPolicyIndex = platformPolicy.generalPolicies?.findIndex((p) => p.reference === policyRef);
