...
export class GitHubPRCreator {
  ...
  async createPolicyPullRequest(
    ...
    try {
      ...
      const prTitle = GitHubPRCreator.generatePRTitle(platformId, forceRewrite, timestamp);
      const prBody = GitHubPRCreator.generatePRBody(platformId, reasoning, forceRewrite, timestamp);
...
`;
   ...
   * Generates PR title
   */
  static generatePRTitle(platformName: string, forceRewrite: boolean, timestamp: string): string {
    return `Policy Update: ${platformName}${forceRewrite ? ' (Replace/rewrite)' : ''} - ${timestamp}`;
  }
}
