...
import { PolicyValidationRequest } from '@/lib/prompts/policy-validation';
      ...
      if (validateChanges) {
        ...
        if (!analysisService.validateValidationRequest(body)) {
          ...
          return NextResponse.json(
            {
              error:
                'Invalid validation request structure. Required: documentUrl, documentReference, documentTitle, originalPolicies, updatedPolicies',
            },
            { status: 400 },
          );
        }
        ...
        const validationResult = await analysisService.validateChanges(
          body as PolicyValidationRequest,
        );
        ...
        if (!analysisService.validateAnalysisRequest(body)) {
          ...
          return NextResponse.json(
            {
              error:
                'Invalid analysis request structure. Required: documentUrl and properly structured scopedPolicies',
            },
            { status: 400 },
          );
        }
