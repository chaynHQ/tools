...
export function RemovalProcess({ onComplete }: RemovalProcessProps) {
  ...
  const [status, setStatus] = useState<ReportingStatus>(formState.reportingInfo?.status || null);
  ...
  const renderProcessSteps = (steps: string[]) => (
    <div className="space-y-0.5">
      {steps.map((step, index) => (
        <div key={index} className="flex items-start gap-3 py-2 pr-4">
  ...
  const getStatusMessage = () => {
    switch (status) {
      case 'standard-completed':
        return {
          message:
            "You've already taken important steps using the standard process. Our Survivor AI can help you generate a formal letter to escalate this issue further.",
          intent: 'suggestion',
        };
      case 'escalated-completed':
        return {
          message:
            "You've already taken important steps using the escalated process. Our Survivor AI can help you generate a formal letter to escalate this issue further.",
          intent: 'suggestion',
        };
      case 'both-completed':
        return {
          message:
            "Sorry to hear that following the processes hasn't worked so far. Our Survivor AI can help you generate a formal letter to escalate this issue further.",
          intent: 'support',
        };
      case 'none-completed':
        return {
          message:
            'You can try these platform processes first, or we can help you create a formal letter right away. Choose whatever approach feels right for your situation.',
          intent: 'guidance',
        };
      default:
        return null;
            ...
            <div className="border rounded-lg">
              <CollapsibleTrigger className="w-full text-left px-4 py-3">
                <div className="flex items-center justify-between">
                  ...
                  <ChevronDown
                    className={`w-5 h-5 transition-transform ${standardOpen ? 'transform rotate-180' : ''}`}
                  />
                </div>
              </CollapsibleTrigger>
              <CollapsibleContent>
            ...
            <div className="border rounded-lg">
              <CollapsibleTrigger className="w-full text-left px-4 py-3">
                <div className="flex items-center justify-between">
                  ...
                  <ChevronDown
                    className={`w-5 h-5 transition-transform ${escalatedOpen ? 'transform rotate-180' : ''}`}
                  />
                </div>
              </CollapsibleTrigger>
              <CollapsibleContent>
